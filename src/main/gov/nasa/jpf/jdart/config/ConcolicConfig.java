...
public class ConcolicConfig {
  ...
  public static enum ExplorationStrategy {
    DFS,
    BFS,
    IN_ORDER
  };
  ...
  public void addConcolicMethod(
      String id, String className, String methodName, AnalysisConfig ac, ParamConfig... params) {
  ...
  private void initialize(Config conf) {
    ...
    this.solver = ConstraintSolverFactory.createSolver(conf);
  ...
  private static StringModel parseStringModel(Config conf) {
    if (conf.hasValue("jdart.stringModel")) {
      try {
        return StringModel.valueOf(conf.getString("jdart.stringModel"));
      } catch (IllegalArgumentException e) {
        logger.warning(
            "Tried to parse StringModel as it is set in the conifg, but got an illegalArgumentException."
                + " "
                + "Will execute with default value.");
      }
    }
    // Default value
    return StringModel.BVModel;
  }
  ...
  public static class MethodConfig {
    ...
    public MethodConfig(String className, String methodName, Class<?>[] paramTypes) {
  ...
  public void addConcolicClass(final String className) {
    ...
    final Predicate<String> inclPred =
