...
public class ConstraintsTree {
  ...
  public ConstraintsTree(
      SolverContext solverCtx,
      AnalysisConfig anaConf,
      ConcolicValues replayValues,
      ConcolicConfig.ExplorationStrategy strategy,
      boolean incremental) {
  ...
  private Node expand(
      LeafNode leaf, Instruction insn, int branchIdx, Expression<Boolean>[] decisions) {
    DecisionNode parent = leaf.parent();
    DecisionNode newInner =
        new DecisionNode(parent, insn, decisions, leaf.childId(), exploreMode, strategy);
    if (parent == null) {
      root = newInner;
    } else {
