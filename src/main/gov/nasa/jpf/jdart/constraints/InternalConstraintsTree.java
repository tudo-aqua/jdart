...
public class InternalConstraintsTree {
	...
	private static final class Node {
		...
		public boolean isExhausted() {
			DecisionData dec = decisionData();
			if (dec == null) {
				return (data != null && data.getClass() !=
										DontKnowData.class); // Dont know is not exhausted, all other forms of data are
			}
		...
		public ResultData result(PathResult result) {
			...
			//throw new IllegalStateException("Attempting to finish already explored path (data = " + data.getClass()
			// .getName() + "!");
			return null;
		}
	...
	/**
	 * Retrieves the node in the constraints tree that would be reached using the given valuation.
	 *
	...
	public BranchEffect decision(Instruction insn, int branchIdx, Expression<Boolean>[] decisions) {
		...
		if (current.isExhausted() &&
			!replay) { // FALK: check how exhaustion is computed, maybe replay check is not necessary
			diverged = true;
			return BranchEffect.INCONCLUSIVE;
		}
