...
public abstract class IFHelper {
	...
	public static Instruction execute(IfInstruction insn, NumericComparator cmp, ThreadInfo ti) {
		...
		Expression<?> exp = checkSymbolicAnnotation(sf);
		if (exp == null) {
			return null;
		}
		...
		if (ConcolicInstructionFactory.DEBUG) {
			ConcolicInstructionFactory.logger.finest(
					"Execute IFEQ: " + v1.conc + " [" + v1.symb + "], symb. result  [" + sat + "]");
		}
		return sat ? insn.getTarget() : insn.getNext(ti);

	}

	private static Expression<?> checkSymbolicAnnotation(StackFrame sf) {
		Expression expr = sf.getOperandAttr(Expression.class);
		if (expr == null) {
			SymbolicNumber sn = sf.getOperandAttr(SymbolicNumber.class);
			if (sn != null) {
				return sn.symbolicNumber;
			} else {
				SymbolicSMTString symbolic = sf.getOperandAttr(SymbolicSMTString.class);
				if (symbolic != null) {
