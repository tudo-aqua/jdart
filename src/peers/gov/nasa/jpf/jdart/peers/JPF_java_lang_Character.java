...
/**
 * JDart version of the native peer class for {@link java.lang.Character}. Refer to {@link JPF_java_lang_Integer} for
 * the rationale behind this redefinition.
 */
public class JPF_java_lang_Character extends gov.nasa.jpf.vm.JPF_java_lang_Character {
	...
	@Override
	@MJI
	public boolean isUnicodeIdentifierStart__C__Z(MJIEnv env, int clsObjRef, char c) {
		// TODO Auto-generated method stub
	...
	public char toLowerCase__C__C(MJIEnv env, int clsObjRef, char c) {
		...
		if (attrs != null) {
			for (Object attr : attrs) {
				if (attr instanceof SymbolicSMTString) {
					env.setReturnAttribute(StringCompoundExpression.createToLower(((SymbolicSMTString) attr).symbolicValue));
				} else if (attr instanceof StringCompoundExpression) {
					env.setReturnAttribute(StringCompoundExpression.createToLower((Expression<String>) attr));
				}
			}
		}
	...
	public char toTitleCase__C__C(MJIEnv env, int clsObjRef, char c) {
		...
		if (attrs != null) {
			for (Object attr : attrs) {
				if (attr instanceof SymbolicSMTString) {
					throw new UnsupportedOperationException("Cannot model toTileCase symbolically");
				} else if (attr instanceof StringCompoundExpression) {
					throw new UnsupportedOperationException("Cannot model toTileCase symbolically");
				}
			}
	...
	public char toUpperCase__C__C(MJIEnv env, int clsObjRef, char c) {
		...
		if (attrs != null) {
			for (Object attr : attrs) {
				if (attr instanceof SymbolicSMTString) {
					env.setReturnAttribute(StringCompoundExpression.createToUpper(((SymbolicSMTString) attr).symbolicValue));
				} else if (attr instanceof StringCompoundExpression) {
					env.setReturnAttribute(StringCompoundExpression.createToUpper((Expression<String>) attr));
				}
			}
		}
