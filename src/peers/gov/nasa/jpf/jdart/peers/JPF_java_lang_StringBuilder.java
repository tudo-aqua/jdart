...
public class JPF_java_lang_StringBuilder extends gov.nasa.jpf.vm.JPF_java_lang_StringBuilder {
   ...
   */ public int append__Ljava_lang_String_2__Ljava_lang_StringBuilder_2(
      MJIEnv env, int objref, int sref) {
    ...
    ConcolicConfig.StringModel sm = ca.getStringModel();
    switch (sm) {
      case BVModel:
        return bvStringPeer.append__Ljava_lang_String_2__Ljava_lang_StringBuilder_2(
            env, objref, sref);
      case SMTLibModel:
        return smtStringPeer.append__Ljava_lang_String_2__Ljava_lang_StringBuilder_2(
            env, objref, sref);
      default:
        throw new IllegalArgumentException(
            "Cannot resolve the String model used inside StringBuilder append"
                + ".\n"
                + "Got unsupported model class: "
                + sm.getClass());
    }
  }

  public int super_append__Ljava_lang_String_2__Ljava_lang_StringBuilder_2(
      MJIEnv env, int objref, int sref) {
    return super.append__Ljava_lang_String_2__Ljava_lang_StringBuilder_2(env, objref, sref);
  }
  ...
  public char charAt__I__C(MJIEnv env, int objref, int index) {
    ConcolicConfig.StringModel sm =
        ConcolicMethodExplorer.getCurrentAnalysis(env.getThreadInfo()).getStringModel();
    switch (sm) {
      case BVModel:
  ...
  @MJI
  @SymbolicPeer
  public void getChars__II_3CI__V(
      MJIEnv env, int objref, int start, int end, int dest, int dest_start) {
    ConcolicConfig.StringModel sm =
        ConcolicMethodExplorer.getCurrentAnalysis(env.getThreadInfo()).getStringModel();
    switch (sm) {
      case BVModel:
